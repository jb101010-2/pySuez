...
class ConsumptionIndexContentResult:
  def __init__(self,
    ...
    self.valeurAncienIndex = valeurAncienIndex
    self.volume = volume
...
class SuezData:
  ...
  def get_quality(self) -> QualityResult:
    ...
    data = self.client.get(INFORMATION_ENDPOINT_QUALITY + contract.inseeCode,
                           need_connection=False)
    quality = QualityResult(**data.json())
    return quality

  def get_interventions(self) -> InterventionResult:
  ...
  def get_limestone(self) -> LimestoneResult:
